      ...
      <div className="glass-panel" style={{ padding: '30px' }}>
        {loading ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            {notifications.map(n => (
              <div key={n.id || n._id} style={{ display: 'flex', gap: '16px', background: 'rgba(255,255,255,0.04)', padding: '20px', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.08)' }}>
                <div style={{ padding: '12px', background: n.type === 'alert' ? 'rgba(243, 156, 18, 0.15)' : 'rgba(0, 201, 167, 0.15)', borderRadius: '12px', color: n.type === 'alert' ? '#F39C12' : '#00C9A7', height: 'fit-content' }}>
                  {n.type === 'alert' ? <AlertCircle size={24} /> : <ShieldCheck size={24} />}
                </div>
                <div>
                  <h4 style={{ fontSize: '16px', fontWeight: '800', marginBottom: '4px' }}>{n.title}</h4>
